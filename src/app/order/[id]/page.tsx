...
function OrderTrackingContent() {
  ...
  // Exchange and Cancel modal states
  ...
  async function loadOrder() {
    ...
    try {
      const url = token
        ? `/api/orders/${cleanId}?token=${token}`
        : `/api/orders/${cleanId}`;
  ...
  return (
    <main className="pt-24 sm:pt-28 md:pt-32 pb-20 max-w-4xl mx-auto w-full px-4 sm:px-6 space-y-6">
      ...
      <div className="bg-white p-6 border border-[#EBD9C8] rounded-sm shadow-2xs space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isCancellable && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-red-50 text-red-700 hover:text-red-800 border border-red-200 hover:border-red-300 rounded text-xs uppercase tracking-wider font-semibold transition-colors cursor-pointer shadow-2xs"
                title="Cancel Order"
              >
            ...
            <span
              className={`px-2.5 py-1 text-xs uppercase tracking-wider font-semibold rounded border ${
        ...
        {order.fulfilmentStatus === "CANCELLED" ? (
          <div className="bg-red-50 p-4.5 rounded border border-red-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center text-red-700 flex-shrink-0">
                <XCircle className="w-5 h-5" />
              </div>
              <div>
                <div className="text-xs uppercase tracking-wider font-semibold text-red-900">
                  Order Cancelled
                </div>
                <div className="text-xs text-red-800 mt-0.5">
                  {order.paymentStatus === "REFUNDED"
                    ? "Payment refund has been initiated to your original payment method."
          ...
          {order.items.map((item: any) => {
            ...
            return (
              <div key={item.id} className="py-4 flex flex-col sm:flex-row gap-4 first:pt-2 last:pb-0">
                ...
                {/* Exchange Button Action (Only available after delivery) */}
                <div className="flex sm:flex-col justify-end items-end gap-2 pt-2 sm:pt-0">
                  {activeExchange ? (
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-[#855D25] text-[#855D25] hover:bg-[#855D25] hover:text-white rounded text-[11px] uppercase tracking-wider font-semibold transition-colors shadow-2xs cursor-pointer"
                    >
      ...
      {/* ── 4. CUSTOMER ORDER ACTIONS & CANCELLATION BAR ── */}
      <div className="bg-white p-5 sm:p-6 border border-[#EBD9C8] rounded-sm shadow-2xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {isCancellable && (
            <button
              type="button"
              onClick={() => setIsCancelModalOpen(true)}
              className="px-4 py-2 bg-white hover:bg-red-50 text-red-700 hover:text-red-800 border border-red-300 rounded text-xs uppercase tracking-wider font-semibold transition-colors cursor-pointer shadow-xs flex items-center gap-1.5"
            >
              <XCircle className="w-3.5 h-3.5 text-red-600" />
              <span>Cancel Order</span>
            </button>
          ...
          <a
            href="https://wa.me/919876543210?text=Hello%20Rajwadi%20Concierge,%20I%20need%20assistance%20with%20Order%20"
            target="_blank"
            rel="noopener noreferrer"
            className="px-3.5 py-2 bg-[#FAF5EE] hover:bg-[#F3EBE1] text-[#855D25] hover:text-[#6D1A2A] border border-[#EBD9C8] rounded text-xs uppercase tracking-wider font-semibold transition-colors cursor-pointer flex items-center gap-1.5"
          >
