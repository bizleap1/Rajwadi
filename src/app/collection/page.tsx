...
import { ArrowRight, ChevronRight, Heart, ShoppingBag, ChevronDown, X } from "lucide-react";
...
type CategoryFilter = "ALL" | "STITCHED" | "UNSTITCHED" | "TRADITIONAL";
...
// Price brackets reflecting actual product price range (₹21,000 – ₹35,000)
const PRICE_FILTERS: PriceFilterItem[] = [
  {
    id: "under-25k",
    label: "Under ₹25,000",
    match: (price) => price < 25000,
  },
  {
    id: "25k-30k",
    label: "₹25,000 – ₹30,000",
    match: (price) => price >= 25000 && price <= 30000,
  },
  {
    id: "above-30k",
    label: "Above ₹30,000",
    match: (price) => price > 30000,
  },
];
...
function CollectionContent() {
  ...
  useEffect(() => {
    if (categoryQuery) {
      ...
      if (["STITCHED", "UNSTITCHED", "TRADITIONAL"].includes(upper)) {
        setActiveCategory(upper as CategoryFilter);
      }
    }
  }, [categoryQuery]);

  // Accordions for compact sidebar height (Colour open by default, Price collapsed by default)
  ...
  const [isMobileColorOpen, setIsMobileColorOpen] = useState(true);
  ...
  // Exact Rajwadi categories with counts
      ...
      () => [
        { key: "ALL", label: "All", count: REAL_POSHAKS.length },
        {
          ...
          count: REAL_POSHAKS.filter(
            (p) => p.category.toUpperCase() === "STITCHED"
          ).length,
        },
        {
          key: "UNSTITCHED",
          label: "Unstitched",
          count: REAL_POSHAKS.filter(
            (p) => p.category.toUpperCase() === "UNSTITCHED"
          ).length,
        },
        {
          key: "TRADITIONAL",
          label: "Traditional",
          count: REAL_POSHAKS.filter(
            (p) => p.category.toUpperCase() === "TRADITIONAL"
          ).length,
        },
      ],
      []
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (selectedPriceRange) {
      ...
      if (priceOpt) {
        list = list.filter((p) => priceOpt.match(parsePrice(p.price)));
      }
    }
    ...
    return sorted;
  }, [activeCategory, selectedColor, selectedPriceRange, sortBy, searchQuery]);
  ...
  return (
    <main className="min-h-screen flex flex-col bg-[#FDFBF7] text-[#171717] selection:bg-[#5A1F2B] selection:text-[#FAF6F0]">
        ...
        <section className="relative pt-28 sm:pt-32 md:pt-36 pb-10 sm:pb-12 md:pb-14 overflow-hidden bg-[#1A070B] text-[#FAF6F0]">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 md:px-12 text-center">
            ...
            >
              ...
              <span className="text-[#C6A15B] font-medium">Collection</span>
            </motion.nav>
        ...
        <div
          className={`lg:hidden sticky z-40 w-full bg-[#FAF6F0] border-y border-[#E6DCB8]/70 shadow-[0_2px_10px_rgba(0,0,0,0.03)] transition-[top] duration-300 ease-in-out ${
